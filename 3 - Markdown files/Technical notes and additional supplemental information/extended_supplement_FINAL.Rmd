...
```{r notes2, fig = T, fig.pos = "H", fig.height = 2.5, fig.cap = "\\label{supp.fig.spain} Spanish COVID-19 deaths, with and without smoothing."}
...
```{r percapitaplot, results = 'tex', fig.pos = "H", fig = T, fig.width = 8, fig.height = 6, fig.cap="Estimates of per capita and total COVID-19 mortality from February-September 2020."}
...
a1 = ggplot(e3, aes(x = date, y = rate_per_100k, group = location_name, col = col)) + 
  ...
  geom_text(size = 3, aes(x = as.Date("2020-09-19"), y = rate_per_100k, label = country_lab)) + 
  labs(x = "", y = "", title = "Cumulative deaths per 100K population") + 
...
a2 = ggplot(e2 %>% filter(date<="2020-09-12"), aes(x = date, y = roll7, group = location_name, col = col)) + 
  ...
  geom_text(size = 3, aes(x = as.Date("2020-09-12"), 
                y = roll7,
                label = ifelse(location_name%in%c("United States of America", "Spain", "Israel", "France"), country_lab_roll, ""))) + 
  scale_color_manual(guide = F, values = pal) + 
  scale_x_date(expand = c(0,10)) + 
  scale_y_continuous(labels = comma, expand = c(0,0)) +
  labs(x = "", y = "", title = "Rolling 7-day average of deaths per 100K population") + geom_vline(xintercept = as.Date("2020-05-10"), lty = 2, col = "darkgrey") + geom_vline(xintercept = as.Date("2020-06-07"), lty = 2, col = "darkgrey") + theme_opts
...
b3 = ggplot(e2, aes(x = date, y = count, group = location_name, col = col)) + 
  ...
  geom_text(size = 3, aes(x = as.Date("2020-09-19"), y = count, label = country_lab)) +
  scale_color_manual(guide = F, values = pal) + 
  scale_x_date(expand = c(0,10)) + 
...
b4 = ggplot(e2, aes(x = date, y = count, group = location_name, col = col)) + 
  geom_line() + 
    geom_point(aes(x = as.Date("2020-09-19"), y = (count)), col = "white", size = 5) +
  geom_text(size = 3, aes(x = as.Date("2020-09-19"), label = country_lab)) +
  scale_color_manual(guide = F, values = pal) + 
  scale_x_date(expand = c(0,10)) + 
  scale_y_continuous(labels=comma_format(accuracy = 1),
